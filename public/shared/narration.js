 ...
 *
 * To regenerate audio: node shared/generate-audio.js --class N
 * To regenerate single slide: node shared/generate-audio.js --class N --slide 5
 */
...
(function() {
  ...
  function startAutoNarration() {
    ...
    // Don't start narration in appendix mode
    if (state.appendixMode) {
      console.log('Cannot start narration in appendix mode');
      return;
    }
  ...
  function narrateCurrentSlide() {
    ...
    // Don't narrate in appendix mode
    if (state.appendixMode) {
      return;
    }
  ...
  function showNarrationIndicator(show) {
    ...
    if (show) {
      if (!indicator) {
        ...
        indicator.innerHTML = 'Narration Active';
  ...
  window.Narration = {
    ...
    onSlideChange: function(slideNumber) {
      ...
      if (isNarrating) {
        ...
        // Stop narration if entering appendix mode
        if (state && state.appendixMode) {
          console.log('Stopping narration - entered appendix mode');
          stopNarration();
          showNarrationIndicator(false);
          return;
        }
